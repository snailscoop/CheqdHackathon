 ...
 * @returns {Promise<void>}
 */
async function processNaturalLanguageCommand(ctx, command) {
  try {
    ...
    if (txHashMatch) {
      ...
      // Handle as blockchain transaction
      const unifiedCredentialHandlers = require('./telegram/handlers/unifiedCredentialHandlers');
    ...
    const result = await grokService.processCommand(command, ctx.from.id);
    ...
    logger.error('Error processing natural language command', { error: error.message });
